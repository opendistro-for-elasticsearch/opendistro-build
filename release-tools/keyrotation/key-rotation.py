...
def encrypt_git_key(public_key,secret):
    try:
        ...
        raise logging.exception("Failed to generate public key")
...
def number_of_keys(iam_username):
    try:
        ...
        raise logging.exception("Failed to find number of keys for the " + iam_username)
    return len(keys)

# Lists the Access keys of an IAM user
def list_keys(iam_username):
    try:
        response = iam.meta.client.list_access_keys(
            UserName=iam_username
        )
        logging.info(response['AccessKeyMetadata'])
        print(response['AccessKeyMetadata'])
        if len(response.get('AccessKeyMetadata')) == 1:
            dict = []
            dict.append(
                { 
                'access_key_id': str(response.get('AccessKeyMetadata')[0].get('AccessKeyId')) , 
                'create_date': response.get('AccessKeyMetadata')[0].get('CreateDate') 
                })
            logging.info(dict)
        else:
            dict = []
            ...
            dict.append(
                {
                'access_key_id': str(response.get('AccessKeyMetadata')[1].get('AccessKeyId')) ,
                'create_date': response.get('AccessKeyMetadata')[1].get('CreateDate')
                })
            logging.info(dict)
    except:
        raise logging.exception("Failed to list keys for IAM user : " + iam_username)
    return list(dict)

# Delete the Access key for an IAM user
def delete_key(iam_username,access_key):
    try:
        logging.info("Delete key : " + access_key)
        response = iam.meta.client.delete_access_key( 
            UserName = iam_username,
            AccessKeyId=access_key
            )
        logging.info("Response for the Access key delete : " + str(response))
        if response.get('ResponseMetadata').get('HTTPStatusCode') != 200:
            raise
    except:
        raise logging.exception("Failed to delete key : " + access_key + " For IAM user : " + iam_username)

# Deactiviate the Access key for an IAM user
def deactivate_key(iam_username,access_key):
    try:
        logging.info("Deactivate key : " + access_key)
        ...
        raise logging.exception("Failed to deactivate key : " + access_key + " For IAM user : " + iam_username)
...
def get_public_key(git_repo,git_owner):
    try:
        ...
        raise logging.exception("Failed to get public key for GIT owner : " + git_owner)
...
def update_git_secret(git_repo,git_owner,git_secret_name,secret):
    try:
        ...
        logging.info("Response code for the GIT Secret update : " + str(response.status_code))
        if response.status_code != 204:
            raise
    except:
        raise logging.exception("Failed to update GIT secret : " + git_secret_name)
...
def create_key(iam_username,git_repo,git_owner,git_access_id,git_access_secret):
    try:
        ...
        raise logging.exception("Failed to create key for IAM user")
...
def main():
    try:
        ...
        with open(sys.argv[1]) as source:
            ...
            for repo in repos:
                try:
                    print("Key rotation is in progress for Repo : " + repo.get("repo"))
                    ...
                    if num_of_keys == 0:
                        ...
                        access_keys = list_keys(repo.get("aws_iam_user"))
                        logging.info(access_keys)
                        create_key(repo.get("aws_iam_user"),git_repo,git_owner,repo.get("git_access_key_id"),repo.get("git_access_secret_key"))
                        deactivate_key(repo.get("aws_iam_user"),access_keys[0].get('access_key_id'))
                    else:
                        access_keys = list_keys(repo.get("aws_iam_user"))
                        logging.info(access_keys)
                        logging.info("Delete Access key : " + access_keys[0].get('access_key_id'))
                        print("Delete Access key : " + access_keys[0].get('access_key_id'))
                        delete_key(repo.get("aws_iam_user"),access_keys[0].get('access_key_id'))
                        create_key(repo.get("aws_iam_user"),git_repo,git_owner,repo.get("git_access_key_id"),repo.get("git_access_secret_key"))
                        deactivate_key(repo.get("aws_iam_user"),access_keys[1].get('access_key_id'))
                    logging.info("Keyrotation completed for Repo : " + git_repo)
                    print("Keyrotation completed for Repo : " + git_repo)
                except:
                    logging.exception("Failed to rotate keys for the Repo : " + git_repo)
