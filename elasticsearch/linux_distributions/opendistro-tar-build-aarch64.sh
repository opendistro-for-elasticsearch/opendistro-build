...
do
  ...
  if [ "$plugin_path" != "none" ] && !(echo $plugin_path | grep -q "opendistro_performance_analyzer")
  then
    echo "installing $plugin_latest"
    $PACKAGE_NAME-$OD_VERSION/bin/elasticsearch-plugin install --batch "${ARTIFACTS_URL}/${plugin_latest}"; \
  else
    echo installing opendistro_performance_analyzer fix
    $PACKAGE_NAME-$OD_VERSION/bin/elasticsearch-plugin install --batch "${ARTIFACTS_URL}/aarch64-test/opendistro_performance_analyzer-1.11.0.0-clifix.zip"; \
  fi
done
